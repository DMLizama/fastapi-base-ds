...
def update_comerciante(
    comerciante_id: int, comerciante: schemas.ComercianteUpdate, db: Session = Depends(get_db)
):
    return services.modificar_comerciante(db, comerciante_id, comerciante)


@router.delete("/{comerciante_id}", response_model=schemas.ComercianteDelete)
